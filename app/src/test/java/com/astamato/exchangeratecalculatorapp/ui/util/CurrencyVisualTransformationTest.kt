...
class CurrencyVisualTransformationTest {
  ...
  private fun transform(input: String): String = transformation.filter(AnnotatedString(input)).text.text

  private fun originalToTransformed(
    input: String,
    offset: Int
  ): Int = transformation.filter(AnnotatedString(input)).offsetMapping.originalToTransformed(offset)

  private fun transformedToOriginal(
    input: String,
    offset: Int
  ): Int = transformation.filter(AnnotatedString(input)).offsetMapping.transformedToOriginal(offset)
